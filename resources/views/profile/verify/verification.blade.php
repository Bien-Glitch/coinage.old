		...
		<div class="nk-kycfm">
			...
			<div class="nk-kycfm-head">
				<a {!! !$user->hasVerifiedBank() ? 'href="' . route('profile.verify.bank') . '"' : NULL !!}>
					<div class=""><img class="w-70 d-block d-sm-none" src="../../../dashboard/images/email/kyc-{{$user->hasVerifiedBank() ? 'success' : 'pending'}}.png" alt="" srcset=""></div>
					{{-- Hidden on only sm --}}
					<div class=""><img class="w-40 	 d-none d-sm-block" src="../../../dashboard/images/email/kyc-{{$user->hasVerifiedBank() ? 'success' : 'pending'}}.png" alt="" srcset=""></div>
				</a>
				<div class="nk-kycfm-title">
					<h5 class="title">Bank Verification</h5>
					<p class="sub-title">
						{{ $user->hasVerifiedBank() ? 'Your Bank Account has been successfully verified.' : 'Your Bank Account still requires verification for your identification.' }}
					</p>
				</div>
			</div><!-- .nk-kycfm-head -->
